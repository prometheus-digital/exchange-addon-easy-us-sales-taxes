...
function showTic() {
    if (ct.length != 5) {
        ...
        jQuery('#catList').html(""); jQuery('#jqTicFinal').html("<input type='hidden' id='" + fieldID + "' name='it-exchange-add-on-easy-us-sales-taxes-us-tic' value='" + ct + "'/>"); jQuery('#ticComplete').html("");
...
function getTic(whichTic) {
    if (whichTic.value == 'reset') {
        ...
        if (selectedObj.tic.children) {
            ...
            jQuery('#catList').append('<span>' + selectedLabel + '</span>');
            jQuery('#jqTicFinal').html("<input type='hidden' id='" + fieldID + "' name='it-exchange-add-on-easy-us-sales-taxes-us-tic' value='" + selectedValue + "'/>");
        }
    }
}