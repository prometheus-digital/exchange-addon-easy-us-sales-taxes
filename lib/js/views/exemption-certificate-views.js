...
(function ($) {
	...
	AUSTCertManager.CertificateView = Backbone.View.extend({
		...
		useCert : function ( event ) {
			...
			this.useCertificate( this.model.id ).done( function( data ) {
				...
				$( '#it-exchange-advanced-us-taxes-exemption-manager', AUSTCertManager.ListCertsView.$el ).scrollTop(0);
		...
		*/
		...
		saveCert : function ( event ) {
			...
			this.addCertificate(newCert).done( function( data ) {
				...
				$( '#it-exchange-advanced-us-taxes-exemption-manager', self.$el ).scrollTop(0);
	...
	function it_exchange_aust_refresh_cart() {
		if ( 'undefined' !== typeof ITExchangeAdvancedUSTaxesCheckoutPage && ITExchangeAdvancedUSTaxesCheckoutPage ) {
